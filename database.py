...
class Clientes:
    ...
    lista = []
    # bloque with para abrir el archivo CSV
    with open(config.DATABASE_PATH, 'r',newline='\n') as portador:
        # extract objeto de tipo reader acceder datos archivo
        extract = csv.reader(portador, delimiter=';')
        #extract = csv.DictReader(portador, delimiter=';')
        for dni, nombre, apellido in extract:
            cliente = Cliente(dni,nombre, apellido)
            #lista.append(str(cliente))
